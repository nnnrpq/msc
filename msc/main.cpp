...
cvReleaseCapture(&capture); cvDestroyWindow("P2");
  ...
  /// Check whether the image belongs to Caltech-101 dataset
    if(argc == 3){
        if(strcmp(argv[2],"Caltech-101") != 0){
            printf("The format for the third argument should be Caltech-101 fllowed by Image_Name/Image_#\n");
            return -1;
        }else{
           input_image = strcat(c, argv[1]);
        }
    }else{
        input_image = argv[1];
    }
  ...
  /// Read the count of memory images for MSC.
    // open a file in read mode.
    
    //int File_exist =  access("Memory_Image_Count.txt", F_OK);
    
    // If the file exists then read the count of memory images.
    // Else we have add the image to the memory and update the file.
    /*
    if (File_exist == 0) {
        fp = fopen( "Memory_Image_Count.txt", "r" );
        fscanf(fp, "%d", &buf);
        fclose(fp);
    }else{
        buf = 0;
    ...
    */
    ...
    if(buf == 0){
        ...
        fp = fopen( "Memory_Image_Count.txt", "w" );
        fprintf(fp, "%d", buf);
        fclose(fp);
    }else{
	...
	// Step 2:- Scroll through all of the images and perform the pixel normalization. There are couple of ways to approach this
        ...
        maxRows = Edge_Detected_Image_unpadded.rows;
        maxCols = Edge_Detected_Image_unpadded.cols;
